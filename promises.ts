...
let performUpload = function(imgStatus : string) : Promise<{imgStatus : string}> { // Step 2: performUpload() is started
  console.log('performUpload called');
  // Promise expects a callback function, called 'resolve' here (can be named anything)
  return new Promise((resolve) => { // Step 3: run 'resolve'
...
.then((performUploadResponse) => { // Step 5: Run funtion
...
// Sequence Overview: performUpload called, Resolve, Promise Resolved message, 'Then' block, restart
